...
function drawList(data){
	...
	for(i=0;i<list.length;i++){ 
		if(list[i].level == '1' || list[i].level == '2' ){
			...
			if(list[i].level == '1'){ //레벨 
				content += '<td>일반 관리자</td>';	
				content += '<td>';
				content += '<div class="form-check form-switch" style="margin-left: 3.5em !important;"><input class="form-check-input" type="checkbox" onchange="authUpdate(this)" value="2" id="'+list[i].mem_id+'" checked></div>'; 
				content += '</td>'; 
			} else if(list[i].level == '2'){ //레벨 
				content += '<td>사원</td>';	
				content += '<td>';
				content += '<div class="form-check form-switch" style="margin-left: 3.5em !important;"><input class="form-check-input" type="checkbox" onchange="authUpdate(this)" value="1" id="'+list[i].mem_id+'"></div>';
				content += '</td>';
			} 
			content += '</tr>';				
		} 
...
// 권한 업데이트 
function authUpdate(elem){
	var bool = confirm("해당 사원의 권한을 수정하시겠습니까?");
	var mem_id = $(elem).attr('id');
	var memAuth = $(elem).val(); 
	
	if(bool){ 
		// 업데이트 요청 보내기 
		$.ajax({
			type:'GET',
			url: '/admin/memAuthUpdate.ajax',
			dataType:'JSON',
			data:{mem_id, memAuth},
			success:function(data){
				console.log(data);  
