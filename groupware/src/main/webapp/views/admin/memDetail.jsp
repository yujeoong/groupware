			...
			<div class="layout-page">
				...
				<div class="content-wrapper">
					...
					<div class="container-xxl flex-grow-1 container-p-y">
								...
								<div class="card-body">
									...
									<div>
										<button type="button" class="btn btn-outline-primary"
											onclick="location.href='/admin/memUpdate.move?mem_id=${member.mem_id}'"
									...
									<div class="table-responsive text-nowrap logTables">
										...
										<button type="button" class="btn btn-sm btn-outline-primary"
											onclick="authWrite()" style="float: right;">추가</button>
										<p>&nbsp;</p>
										<div class="table-responsive text-nowrap"
											style="height: 200px">
											<table class="table table-hover" style="text-align: center;">
									...
									<div class="table-responsive text-nowrap logTables">
										...
										<button type="button" class="btn btn-sm btn-outline-primary" 
											onclick="location.href='/schedule/adminScheWriteForm.move?mem_id=${member.mem_id}'" style="float: right;">스케줄러 이동</button>
										<p>&nbsp;</p>      
										<div class="table-responsive text-nowrap"
											style="height: 300px">
											<table class="table table-hover" style="text-align: center;">
						...
						<!-- 모달 조회권한 등록 modalAuthWrite-->
						<div class="modal fade" id="modalCenter_auth" tabindex="-1">
							<div class="modal-dialog">
								<form class="modal-content">
									<div class="modal-header" style="flex-direction: row-reverse">
										<button type="button" class="btn-close"
											data-bs-dismiss="modal" aria-label="Close"></button>
										<h5 class="modal-title" id="modalCenterTitle_Auth"></h5>
									</div>
									<div class="modal-body" id="modalAuthWrite"></div>
								</form>
							</div>
						</div>
...
<script>
			...
			for (var i = 0; i < list.length; i++) {
				if (list[i].category == "1") {
					...
					content += '<td><button type="button" id="'
							+ list[i].num
							+ '" onclick="authDelete(this)" class="btn btn-xs float-end">삭제</button>';
					content += '</tr>';
	...
	function authWrite() {
		var content = '';
		console.log("조회권한 등록 요청");

		content += '<div class="mb-3 row">';
		content += '<label for="html5-datetime-local-input" class="col-md-2 col-form-label">카테고리</label>';
		content += '<div class="col-md-9" style="margin: 0 auto;">';
		content += '<select id="category" class="form-select" onchange="cateSelect()">';
		content += '<option value="nosel" selected>선택하세요.</option>';
		content += '<option value="1">부서</option>';
		content += '<option value="2">프로젝트</option>';
		content += '<option value="3">아티스트/연습생</option>';
		content += '</select>';
		content += '</div></div>';
		content += '<div class="mb-3 row">';
		content += '<label for="html5-datetime-local-input" class="col-md-2 col-form-label">분류</label>';
		content += '<div class="col-md-9" style="margin: 0 auto;">';
		content += '<select class="form-select" id="num">';
		content += '<option id="numList" value="nosel" selected>선택하세요.</option>';
		content += '</select>';
		content += '</div></div>';
		content += '<div class="mb-3 row">';
		content += '<label for="html5-datetime-local-input" class="col-md-2 col-form-label">참여 권한</label>';
		content += '<div class="col-md-9" style="margin: 0 auto;">';
		content += '<select class="form-select" id="parti">';
		content += '<option id="authList" value="Y">Y</option>';
		content += '<option id="authList" value="N" selected>N</option>';
		content += '</select>';
		content += '</div></div>';
		content += '</div><p>&nbsp;</p>';
		content += '<div class="float-end" style="margin:auto;">';
		content += '<button type="button" onclick="authRegist()" class="btn btn-primary" style="width: 100px;">추가</button>'
				+ '<button type="button" data-bs-dismiss="modal" class="btn btn-outline-primary" style="margin-left: 10px; width: 100px;">취소</button>'
		content += '</div>';
		$('#modalCenterTitle_Auth').html("조회 권한 추가");
		$('#modalAuthWrite').empty();
		$('#modalAuthWrite').append(content);
		$('#modalCenter_auth').modal("show");
	}
	...
	function cateSelect() {
		...
		if (category == 3) {
			content += '<option id="numList" value="0" selected>-</option>';
			$('#num').empty();
			$('#num').append(content);
		} else if (category == 'nosel') {
			content += '<option id="numList" selected>선택하세요.</option>';
			$('#num').empty();
			$('#num').append(content);
		} else {
			$
						...
						success : function(data) {
							...
							if (category == 1) {
								for (var i = 0; i < cateList.length; i++) {
									content += '<option id="'+cateList[i].dep_idx+'" value="'+cateList[i].dep_idx+'">'
											+ cateList[i].name + '</option>';
								}
								$('#num').empty();
								$('#num').append(content);
							} else if (category == 2) {
								for (var i = 0; i < cateList.length; i++) {
									content += '<option id="'+cateList[i].prj_idx+'" value="'+cateList[i].prj_idx+'">'
											+ cateList[i].prj_subject
											+ '</option>';
								}
								$('#num').empty();
								$('#num').append(content);
	...
	function authRegist() {
		...
		$num = $("select[id='num']");
		$parti = $("select[id='parti']");
		console.log($category.val());
		console.log($num.val());
		console.log($parti.val());

		// 공란 여부 확인 
		if ($category.val() == 'nosel') {
			alert('카테고리를 선택해 주세요.');
		} else if ($num.val() == 'nosel') {
			alert('분류를 선택해 주세요.');
		} else {
			console.log('서버로 전송');
