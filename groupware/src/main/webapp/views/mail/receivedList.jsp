        ...
        <div class="layout-page">
          ...
          <div class="content-wrapper">
			...
			<div class="container-xxl flex-grow-1 container-p-y">
				<h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">쪽지 /</span> 받은쪽지함</h4>
 				
				<br>

				<nav aria-label="breadcrumb">
                    <a href="./receivedList.move">
                        <span>받은쪽지함 </span>                    
					</a>						
					<a class="breadcrumb-item" href="./sentList.move">
						<span> / 보낸쪽지함</span>
					</a>						
				...
				<div align="right">
					<button type="button" onclick="read()" class="btn btn-outline-primary" id="btnRead">읽음</button>
				...
				<div class="input-group input-group-merge">
					...
					<input type="text" class="form-control" id="searchInput" placeholder="검색어를 입력하세요." aria-describedby="basic-addon-search31">&nbsp;
					<button type = "button" id="search_mail" class="btn btn-outline-primary" onclick="search_mail()">검색</button>
				</div>
				-->
...
function listCall(page, cat, searchType, searchInput) {
	...
	$.ajax({
		type : 'post', 
		url : '/mail/list.ajax',
		data : {page:page, cat:1, searchType:searchType, searchInput:searchInput},
		dataType : 'JSON', 
...
function drawList(list) {
	...
	if(list.length < 1){
		content += '<tr>';
		content += '<td colspan = "4">검색 조건에 만족하는 결과가 없습니다.</td>';
		content += '</tr>';
	...
	for (var i = 0; i < list.length; i++) {
		...
		content += '<td><input type="checkbox" id="checkbox" value="'+list[i].mail_idx+'" /></td>'; 
		content += '<td>'+list[i].dept_name+' '+list[i].mem_name+' '+list[i].pos_name+'</td>'; 
		
		if(list[i].state == 'Y'){//읽었으면
			content += '<td class="td_subject"><a style = "color:#697a8d;" href="detail.move?mail_idx='+list[i].mail_idx+'">'+list[i].title+'</a></td>'; 
		}else{//안읽었으면
			content += '<td class="td_subject"><a style="font-weight : bold;" href="detail.move?mail_idx='+list[i].mail_idx+'">'+list[i].title+'</a></td>'; 
		}
...
//읽음처리 
function read(){
	var state = confirm("쪽지를 읽음으로 변경하시겠습니까?");
	if(!state){
		return false;
	}
	var chkArr = [];
