...
<body>
	...
	<div class="mt-3">
		...
		<div class="modal fade" id="notiModal" tabindex="-1"
			aria-hidden="true">
			<div class="modal-dialog float-end" role="document">
				<div class="modal-content">
					<div class="modal-header">
<!-- 						<h5 class="modal-title" id="notiInBox">알림</h5> -->
							...
							<ul class="nav nav-tabs" role="tablist">
								<li class="nav-item">
									<button type="button" class="nav-link active" role="tab"
										...
										aria-controls="navs-justified-home" aria-selected="true" onclick="notiInBoxAjax()">
										<i class="tf-icons bx bx-home"></i>수신 알림 <span
											class="badge rounded-pill badge-center h-px-20 w-px-20 bg-label-danger" id="unreadCount"></span>
									</button>
								</li>
								<li class="nav-item">
									<button type="button" class="nav-link" role="tab"
										data-bs-toggle="tab" data-bs-target="#navs-justified-profile"
										aria-controls="navs-justified-profile" aria-selected="false" onclick="notiSentListAjax()">
...
<script>
	...
	function drawNotiInBox(list) {
		...
		for (var i = 0; i < list.length; i++) {
			...
			content += '<small> 발신자 ' + list[i].dep_name +'&nbsp'+ list[i].name +'&nbsp'+ list[i].pos_name +'</small>';
		...
		for (var i = 0; i < list.length; i++) {
			...
			for (var j = 0; j <list[i].rcpList.length; j++) {
				if(list[i].rcpList[j].state == 'Y'){
					read_rcp_count +=1;
					read_rcp += '<li class="dropdown-item">'+list[i].rcpList[j].dep_name+'&nbsp'+list[i].rcpList[j].name+'&nbsp'+list[i].rcpList[j].pos_name+'&nbsp<i class="bx bx-check"></i></li>';
				}else{		
					unread_rcp += '<li class="dropdown-item">'+list[i].rcpList[j].dep_name+'&nbsp'+list[i].rcpList[j].name+'&nbsp'+list[i].rcpList[j].pos_name+'&nbsp</li>';
				}
			}
			...
			content += '수신확인 '+read_rcp_count+'/'+list[i].rcpList.length+'</button>';
	...
	$(document).on('click', '.noti-unread', function() {
		...
		$.ajax({
			...
			success : function(data) {
				if (data > 0) {
					if(this.id != 'readAll'){
						$(this).removeClass('list-group-item-primary noti-unread');
						unread -=1;
					}else{
						$('.noti-unread').removeClass('list-group-item-primary noti-unread');
						unread = 0;
					}
						$('#unreadCount').text(unread);
				}
