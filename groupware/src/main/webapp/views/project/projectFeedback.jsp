...
function feedBackListCall(){
var prj_idx = ${prj_idx}
	$.ajax({
		type:'GET',
		url:'/project/feedbackList.ajax',
		data:{prj_idx: prj_idx},
		dataType:'JSON',
		success: function(data){
			console.log(data);
			feedBackdrawList(data)
		},
		error: function(e){
			console.log("에러 : " + e);
		}
...
function feedBackdrawList(data){
	var post = data.post;
...
//일반글 댓글 리스트 출력
function feedbackCommListCall(elem){
...
//피드백 수정 상세보기
function feedbackDetail(elem){
	var prj_idx = ${prj_idx}
	var prj_post_idx = $(elem).closest('#closest').find('#prj_post_Idx').val();
		$.ajax({
			type:'GET',
			url:'/project/feedbackDetail.ajax',
			data:{prj_idx: prj_idx, prj_post_idx: prj_post_idx},
			dataType:'JSON',
			success: function(data){
				console.log(data);			
				$("#feedback_subject").val(data.subject);
				$("#feedback_content").html(data.content);
				$("#feedback_prj_post_idx").val(data.prj_post_idx);				
				$("#feedback_regist_form").attr("action","/project/feedbackUpdate.do");
			},
			error: function(e){
				console.log("에러 : " + e);
			}
...
function feedbackUpdate(){
	console.log('피드백수정하자고오ㅗ오');
    //var tog = $("#toggle1").html();
	var subject = $(".subject").val()
	var content = $(".content").val()
	var prj_idx = ${prj_idx}
	var prj_post_Idx= ${prj_post_Idx}
	$.ajax({
		type: 'GET',
		url: '/project/feedbackUpdate.ajax',
...
console.log(comment);
	var content = '<input type="text" name="upt_comment" style="width: 400px;"  class="upt_comment" value="">';
